...
```{r}
k_grid = seq(5,100,by=10)
cv_grid = foreach(k = k_grid, .combine='rbind') %dopar% {
  models = map(airport_covid_train, ~ knnreg(ARR_DELAY_NEW ~ MONTH + DAY_OF_WEEK + MKT_UNIQUE_CARRIER + ORIGIN + DEST + DISTANCE + cases_avg_per_100kk, k=k, data = ., use.all=FALSE))
  errs = map2_dbl(models, airport_covid_test, modelr::rmse)
