...
```{r}
K_folds = 20
folds = createFolds(hotels_val$children, k=K_folds)

results = lapply(folds, function(x){
  test = hotels_val[x,]
  pred = predict(hotels_baseline2, test)
   return(pred)
})
...
difference = c()
for (k in seq(1, 20)){ 
  predicted = append(predicted, mean(unlist(results[k]))*250)
  difference = append(difference, as.integer(unlist(actual[k])) - as.integer(predicted[k]))
}

final = cbind(predicted, actual, difference)
final
```
