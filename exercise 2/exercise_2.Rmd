...
hotels_prediction = map(hotels_dev_folds_final$test, ~ predict(hotels_baseline2$`1`, data= ., type='response'))

final = foreach(k = sep(1, 20), .combine=rbind) %do% { 
...
final
sum(hotels_prediction$`1`)
sum(hotels_dev_folds_final$test$`1`$data$children)
```
