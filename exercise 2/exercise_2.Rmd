...
models = map(hotels_dev_folds$train, ~ caret::knn3(children ~ market_segment + adults + customer_type + is_repeated_guest, k=2, data = ., use.all=FALSE))
errs = (models, hotels_dev_folds$test, modelr::rmse)
...
cv_grid_hotel_2 = foreach(k = k_grid, .combine='rbind') %dopar% {
  models = map(hotels_dev_folds$train, ~ caret::knn3(children ~ -arrival_date, k=k, data = ., use.all=FALSE))
  errs = map2_dbl(models, hotel_dev_folds$test, modelr::rmse)
  c(k=k, err = mean(errs), std_err = sd(errs)/sqrt(K_folds))
} %>% as.data.frame
