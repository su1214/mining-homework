...
hotels_baseline2 = map(hotels_dev_folds$train, ~ glm(children ~ . - arrival_date, data = ., family = binomial))
...
Specifically, LPM has slightly higher TPR when the threshold is lower(0.2) compared to baseline 2 with little difference in FPR between these two models. When threshold is 0.7, the trade-off of TPR and FPR makes it difficult to tell which one is better.
We choose LPM for further analysis.

### Model validation: step 1

```{r}
hotels_val = read_csv("hotels_val.csv")
phat_val_lpm = predict(hotels_lpm$`1`, hotels_val, type='response')
thresh_grid = seq(0.95, 0.05, by=-0.005)
roc_curve_hotels_val = foreach(thresh = thresh_grid, .combine='rbind') %do% {
  
  yhat_val_lpm = ifelse(phat_val_lpm >= thresh, 1, 0)
  
  confusion_out_lpm = table(y = hotels_val$children, yhat = yhat_val_lpm)

  # FPR, TPR for linear model

  out_lpm = data.frame(model = "lpm",
                         TPR = ifelse(class(try(confusion_out_lpm[2,"1"], silent=TRUE)) == "try-error", 0, confusion_out_lpm[2,"1"]/sum(hotels_val$children==1)),
                         FPR = ifelse(class(try(confusion_out_lpm[1,"1"], silent=TRUE)) == "try-error", 0, confusion_out_lpm[1,"1"]/sum(hotels_val$children==0)))
...
ggplot(roc_curve_hotels_val) +
  ...
  ylim(0,1) + xlim(0,1) +
  labs(title="ROC curve of LPM using hotels_val data")
```
...
hotels_results = lapply(hotels_val_folds, function(x){
  test = hotels_val[x,]
  pred = predict(hotels_lpm$`1`, test)
   return(pred)
})
