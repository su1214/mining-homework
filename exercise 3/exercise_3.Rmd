...
greenbuildings = greenbuildings %>% mutate(green_certified = LEED | Energystar ) %>% mutate(revenue = leasing_rate*Rent)
...
# Linear Regression
green_lm = lm(revenue ~ ., data=green_train)
modelr::rmse(green_lm, green_test)

# Variable Selection stepwise
green_forward = step(green_lm, direction='forward',
                     scope=~(.)^2)
modelr::rmse(green_forward, green_test)

# Trees
green.tree = rpart(revenue ~ ., 
                    data=green_train,
                    control = rpart.control(cp = 0.002, minsplit=30))
